...
class TypingTutorApp:
    ...
    def create_widgets(self):
        ...
        self.custom_time_button = tk.Button(time_buttons_frame, text="Establecer Tiempo", 
                                          command=self.set_custom_time, font=("Inter", 8), 
                                          bg="#17a2b8", fg="white", activebackground="#138496", 
                                          relief="raised", bd=2, cursor="hand2", state=tk.DISABLED)
        self.custom_time_button.pack(side=tk.LEFT, padx=8)

        self.selected_time_label = tk.Label(time_config_frame, text="Tiempo seleccionado: 4 minutos", 
                                          font=("Inter", 8, "bold"), fg="#007BFF", bg="#fff")
        self.selected_time_label.pack(pady=2)

        # --- Área para pegar texto ---
        paste_frame = tk.LabelFrame(scrollable_frame, text="Pega tu texto aquí", 
                                   font=("Inter", 10, "bold"), bg="#fff", fg="#555", 
                                   bd=1, relief="flat", padx=10, pady=10)
        paste_frame.pack(fill=tk.X, pady=(0, 2))

        self.paste_text_area = scrolledtext.ScrolledText(paste_frame, wrap=tk.WORD, width=60, height=2, 
                                                        font=("Inter", 10), bd=1, relief="solid")
        self.paste_text_area.pack(fill=tk.X, pady=(0, 2))

        load_button = tk.Button(paste_frame, text="Cargar Texto para Practicar", command=self.load_text, 
                               font=("Inter", 8, "bold"), bg="#4CAF50", fg="white", 
        ...
        self.text_stats_label = tk.Label(scrollable_frame, text="Estadísticas del texto: No hay texto cargado", 
                                       font=("Inter", 10, "bold"), fg="#555", bg="#f0f0f0")
        self.text_stats_label.pack(pady=2)

        # --- Áreas de tipeo ---
        typing_container = tk.Frame(scrollable_frame, bg="#f0f0f0")
        typing_container.pack(fill=tk.X, pady=(0, 10))
        ...
        self.display_text_area = scrolledtext.ScrolledText(copy_frame, wrap=tk.WORD, width=50, height=20, 
                                                          font=("Inter", 12), state=tk.DISABLED, bd=1, 
        ...
        self.typing_area = scrolledtext.ScrolledText(type_feedback_frame, wrap=tk.WORD, width=50, height=20, 
        ...
        control_section = tk.Frame(scrollable_frame, bg="#f0f0f0", relief="flat", bd=1)
        control_section.pack(fill=tk.X, pady=1, padx=1)
        ...
        self.start_button = tk.Button(buttons_frame, text="🚀 INICIAR PRÁCTICA", command=self.start_typing, 
                                    font=("Inter", 8, "bold"), bg="#007BFF", fg="white", 
        ...
        self.reset_button = tk.Button(buttons_frame, text="🔄 REINICIAR", command=self.reset_session, 
                                    font=("Inter", 8, "bold"), bg="#DC3545", fg="white", 
                                    activebackground="#c82333", relief="raised", bd=3, cursor="hand2", 
                                    padx=5, pady=2)
        self.reset_button.pack(side=tk.LEFT, padx=2)

        # --- ESTADÍSTICAS EN TIEMPO REAL ---
        stats_section = tk.Frame(scrollable_frame, bg="#f0f0f0", relief="solid", bd=1)
        stats_section.pack(fill=tk.X, pady=(0, 5), padx=10)

        stats_grid = tk.Frame(stats_section, bg="#f0f0f0")
        stats_grid.pack(pady=15)

        # Todas las estadísticas en una sola línea
        self.timer_label = tk.Label(stats_grid, text="⏱️ Tiempo: 00:00", font=("Inter", 8, "bold"), 
                                   fg="#333", bg="#f0f0f0")
        self.timer_label.pack(side=tk.LEFT, padx=15)

        self.wpm_label = tk.Label(stats_grid, text="⚡ Velocidad: 0 WPM", font=("Inter", 8, "bold"), 
                                 fg="#333", bg="#f0f0f0")
        self.wpm_label.pack(side=tk.LEFT, padx=15)

        self.target_wpm_label = tk.Label(stats_grid, text="🎯 WPM Objetivo: 0", font=("Inter", 8, "bold"), 
                                       fg="#28a745", bg="#f0f0f0")
        self.target_wpm_label.pack(side=tk.LEFT, padx=15)

        self.correct_words_label = tk.Label(stats_grid, text="✅ Palabras Correctas: 0", 
                                          font=("Inter", 8, "bold"), fg="#333", bg="#f0f0f0")
        self.correct_words_label.pack(side=tk.LEFT, padx=15)
    ...
    def update_target_wpm(self):
        ...
        if self.word_count > 0 and self.selected_time > 0:
            ...
            self.target_wpm_label.config(text=f"🎯 WPM Objetivo: {self.target_wpm:.1f}")
        else:
    ...
    def reset_session(self):
        ...
        self.typing_area.config(state=tk.NORMAL)  # Habilitar antes de limpiar
        self.typing_area.delete("1.0", tk.END)
        self.typing_area.config(state=tk.DISABLED)  # Deshabilitar hasta cargar texto
